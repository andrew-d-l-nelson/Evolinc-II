        ...
        if line == '\n':
            ...
            try:
                try:
                    ...
                    if subject_gff != '':
                        query = "bash /LastBuilding_Families.sh -g %s -l %s -q %s -s %s -f %s -k %s -e %s -n %s" % \
                                (subject, lincRNA, query_spp, subject_spp, subject_gff, known_lincRNAs, value, threads)

                    else:
                        query = 'bash /LastBuilding_Families.sh -g %s -l %s -q %s -s %s -k %s -e %s -n %s' % \
                    ...
                    query = 'bash /LastBuilding_Families.sh -g %s -l %s -q %s -s %s -f %s -e %s -n %s' % \
                            (subject, lincRNA, query_spp, subject_spp, subject_gff, value, threads)
            except IndexError:
                query = 'bash /LastBuilding_Families.sh -g %s -l %s -q %s -s %s -e %s -n %s' % \
