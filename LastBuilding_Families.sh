...
lastdb -P 0 BLAST_DB/$subject_genome.blast.out $subject_genome
...
lastal -E10 -P 0 -f BlastTab+ BLAST_DB/$subject_genome.blast.out $lincRNAfasta > Homology_Search/$subject_species.out
...
#Remove first 16 lines of comments describing output and final comment
sed '/^#/ d' Homology_Search/$subject_species.stripped.out > Homology_Search/$subject_species.stripped.cmtsrmd.out
cat Homology_Search/$subject_species.stripped.cmtsrmd.out > Homology_Search/$subject_species.stripped.out 

# Convert blast result to gff
perl /Lastblast2gff.pl -i Homology_Search/$subject_species.stripped.out -s $subject_species -o Homology_Search/$subject_species.out.gff
...
if [ ! -z $known_lincRNAs ]; then 
    ...
    lastdb -P0 BLAST_DB/$known_lincRNAs.blast.out $known_lincRNAs
...
lastal -E10 -P 0 -f BlastTab+ BLAST_DB/$subject_genome.blast.out Homology_Search/$subject_species.$query_species.orthologs.renamed.fasta > Homology_Search/$subject_species.$query_species.orthologs.renamed.lincRNAs_tested.out

    # Remove spaces in the blastout files
    sed 's/ //g' Homology_Search/$subject_species.$query_species.orthologs.renamed.lincRNAs_tested.out > Homology_Search/$subject_species.$query_species.orthologs.renamed.lincRNAs_tested.stripped.out 

    #Remove first 16 lines of comments describing output and final comment
    sed '/^#/ d' Homology_Search/$subject_species.$query_species.orthologs.renamed.lincRNAs_tested.stripped.out > Homology_Search/$subject_species.$query_species.orthologs.renamed.lincRNAs_tested.stripped.cmtsrmd.out
    cat Homology_Search/$subject_species.$query_species.orthologs.renamed.lincRNAs_tested.stripped.cmtsrmd.out > Homology_Search/$subject_species.$query_species.orthologs.renamed.lincRNAs_tested.out
...
lastal -E10 -P 0 -f BlastTab+ BLAST_DB/$subject_genome.blast.out Homology_Search/$query_species.$subject_species.into_200_bin.fasta > Homology_Search/$subject_species.into_200_bin.fasta.out


# Remove spaces in the blastout files
sed 's/ //g' Homology_Search/$subject_species.into_200_bin.fasta.out > Homology_Search/$subject_species.into_200_bin_stripped.out

#Remove first 16 lines of comments describing output and final line
sed '/^#/ d' Homology_Search/$subject_species.into_200_bin_stripped.out > Homology_Search/$subject_species.into_200_bin_stripped_cmtsrmd.out
cat Homology_Search/$subject_species.into_200_bin_stripped_cmtsrmd.out > Homology_Search/$subject_species.into_200_bin_stripped.out 


# Convert blast result to gff
perl /Lastblast2gff.pl -i Homology_Search/$subject_species.into_200_bin_stripped.out -s $subject_species -o Homology_Search/$subject_species.into_200_bin.gff
