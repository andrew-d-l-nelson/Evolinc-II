...
with open(inputfile_name, 'r') as inpt:
    for line in inpt:
        if line == '\n':
            ...
            #query = "bash /LastReciprocal_BLAST.sh -g %s -s %s -b %s -c %s -e %s -n %s" % \
                            #(genome, sequences, query_species, query_genome, value, threads)
            query = "bash /LastReciprocal_BLAST.sh -g %s -s %s -f %s -a %s -b %s -c %s -e %s -n %s" % \
